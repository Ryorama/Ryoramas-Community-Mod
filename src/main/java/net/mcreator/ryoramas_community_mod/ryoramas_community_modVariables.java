...
public class ryoramas_community_modVariables {
	public static class WorldVariables extends WorldSavedData {
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				ryoramas_community_mod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				ryoramas_community_mod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(world.dimension::getType), new WorldSavedDataSyncMessage(
						1, this));
			}
		}
	...
	public static class MapVariables extends WorldSavedData {
		public static final String DATA_NAME = "ryoramas_community_mod_mapvars";
		...
		public void syncData(World world) {
			this.markDirty();
			if (world.isRemote) {
				ryoramas_community_mod.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				ryoramas_community_mod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	public static class WorldSavedDataSyncMessage {
		...
		private static void syncData(WorldSavedDataSyncMessage message, LogicalSide side, World world) {
			if (side.isServer()) {
				...
				if (message.type == 0) {
					ryoramas_community_mod.PACKET_HANDLER.send(PacketDistributor.ALL.noArg(), message);
					world.getServer().getWorld(DimensionType.OVERWORLD).getSavedData().set(message.data);
				} else {
					ryoramas_community_mod.PACKET_HANDLER.send(PacketDistributor.DIMENSION.with(world.dimension::getType), message);
